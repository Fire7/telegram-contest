...
class Chart2 {
  constructor(el, data, colorTheme) {
    ...
    this.plot = new Plot2(chartCanvas, chartValuesCanvas, {
      DPI,
      lineThickness: 2,
  ...
  render(dt) {
    t0 = performance.now();
    const s = t0;
    this.plot.setPosition(0, 1); //this.controlPlot.left, this.controlPlot.right);
    this.plot.render(dt);
    t1 = performance.now();
...
class Plot2 {
  ...
  constructor(canvasEl, canvasValuesEl, options) {
    ...
    const gl = canvasEl.getContext('2d', {
      alpha: false,
      antialias: true,
  ...
  prepareGL(gl) {
    const vertexShader = webGLUtils.createShader(gl, gl.VERTEX_SHADER, Plot2.VERTEX_SHADER_SRC);
    const fragmentShader = webGLUtils.createShader(gl, gl.FRAGMENT_SHADER, Plot2.FRAGMENT_SHADER_SRC);
  ...
  setPosition(left, right) {
    ...
    if (diff !== this.diff) {
      ...
      let dateBasis =  this.totalItemsInFrame / (this.canvasPos.width / (Plot2.DATE_WIDTH * 8)) >> 0;
  ...
  popupHTML(index) {
    return `
      <div class='date'>${Plot2.DATE_FORMAT.format(this.chartData.timestamps[index])}</div>
      <div class='values'>
  ...
  drawDates(dt) {
    ...
    Object.keys(this.drawDateItemsAnimations).forEach(i => {
      ...
      const x = this.getItemPosition(i) * this.chartWidth - Plot2.DATE_WIDTH;
  ...
  render(dt) {
    ...
    *!/
    ...
    let vertexes = [];
    // TODO: calculate from current level of zoom:
    const startPoint = 0;
    const endPoint = this.chartData.length - 1;

    let normalData;

    let x0, y0, x1, y1;
    let line, lineLength, lineNormal;
    let a,b,c,d;
    ...
    for (let ci = 0; ci < this.chartData.columns.length ; ci++) {
      column =  this.chartData.columns[ci];
      normalData = column.normalData;

      vertexes = [];
      for (i = startPoint + 1; i <= endPoint; i++) {
        x1 = this.getItemPosition(i);
        y1 = normalData[i] * this.yScale[ci];

        x0 = this.getItemPosition(i - 1);
        y0 = normalData[i - 1] * this.yScale[ci];

        line = [x1 - x0, y1 - y0]; // vec2 x1 - vec2 x0
      //  line = [Math.abs(x1 - x0), Math.abs(y1 - y0)];

        line = [ -line[1], line[0]];



        lineLength = Math.sqrt(line[0] * line[0] + line[1] * line[1]);

        const cosLine = line[0] / lineLength;
        const sinLine = Math.sqrt(1 - cosLine * cosLine);

        const tg = sinLine / cosLine;
        const ctg = cosLine / sinLine;
        ...
        lineNormal = [ line[0] / lineLength, line[1] / lineLength];



        thickDelta = [lineNormal[0] * thick, lineNormal[1] * thick]; // vec2 lineNormal * thick;

        a = [x0 - thickDelta[0], y0 - thickDelta[1]];
        b = [x0 + thickDelta[0], y0 + thickDelta[1]];

        c = [x1 - thickDelta[0], y1 - thickDelta[1]];
        d = [x1 + thickDelta[0], y1 + thickDelta[1]];
      ...
      gl.uniform4fv(this.uniforms.color, normalizeColor(column.colorRGB).concat(this.lineOpacity[ci]));
      gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(vertexes), gl.DYNAMIC_DRAW);
      gl.drawArrays(this.gl.TRIANGLES, 0, vertexes.length / 2);
